...
The data undergoes change during the translation from XML to C structs to support ease of use. The structure of the data and relationships of the elements are presented in the figure below.
...
Module\_schedule has also been removed, leaving its attribute majorFrameSeconds as an independent piece of configuration used directly. 
...
Partitions sub elements, sampling and queuing ports, have combined into a single element called ports. 
The Connection\_Table has been removed, and StandardPartition has been transformed into a port reference list called ports\_t which shares an association with partitions\_t ports.
\\\\
part\_mem\_t has two new attributes.
nummemory is
memory\_arr is
\\\\
partition\_schedule has two new attributes.
numWindows is 
windows\_arr is
\\\\
partitions\_t has two new attributes.
nb\_ports is
ports is
\\\\
port\_t has x new attributes.
is\_queuing\_port is
\\\\
channel\_t has two new attributes.
nb\_ports is
ports is
\\\\

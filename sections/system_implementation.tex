...
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=3cm]{hardware/board_with_jtag_lines}
  \caption{The JTAG lines}
  \label{fig:sub1}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=3cm]{hardware/board_with_jtag_and_serial}
...
At a core frequency of 168 MHz, \textit{DWT\_CYCCNT} is
...
\begin{figure}[H]	\includegraphics[clip=true,trim=0cm 20cm 0cm 0cm,width=\linewidth,keepaspectratio]{figures/cstructs.pdf}
	\caption{C structs design and relationships}
	\label{fig:cstructs}
\end{figure}

The global structure of ARINC\_653\_Module (figure \ref{fig:open653schema}) has been removed,
...
Provided that the data in the XML schema is valid, this
...
may create, the \texttt{processes} array has a static size of 3.
...
Partition memory spaces are allocated in the XML schema, and they are parsed by the
...
such as priority, state, stack size, etc. through an anonymous struct (PROCESS\_STATUS\_TYPE) defined in
...
hence the name \emph{tick}Stamp.\\
...
\texttt{partition\_t} data structure. Unallocated processes are identified by
...
OS initialisation, and it is the main function (in \arinc\ terms, the entrypoint),
of a partition. From userspace, processes can create other processes
...
parser. This partition will always be the partition with index 0. The reason this
partition is kept visible in the generated xml\_data.c file, is to make it more
...
\texttt{NOP}-loop. See listing \ref{list:idle_src} to see the source code of the
idle partition.\\
...
resources. While executing, the code uses no memory, although it needs at
least 32 bytes to store the hardware stacked registers during a context switch.
The reason the code consists of three \texttt{NOP} instructions before a branch, is
...
standard C library, with the \_sys being present to indicate that it is a system
partition. User partitions should only interface with the rest of the system
through APEX calls, so not to enter kernel space and directly communicate with
the UART. This partition makes this interface possible by offering a port,
to send strings or raw data over the UART.\\

stdio\_sys\textquotesingle\ entrypoint is \texttt{stdio\_sys\_main()}, which only
starts a new process with the \texttt{worker()} function, shown in listing
\ref{list:stdio_sys}.
...
Lines 36-39 in the listing show how the port \texttt{sys\_stdio} is initialized and lines
43-49 show how messages are read from the port, and how on success the messages
are transmitted on the UART with the \texttt{BSP\_UARTx\_transmit()} function.
...
messages to the stdio\_sys through queuing ports and to toggle the LEDs.
Only \texttt{yellow\_toggler} is explained here, since they both do the same.\\

Just like the \texttt{stdio\_sys} partition in section \ref{impl:stdio_sys}, the
only job of the function\textquotesingle s entrypoint is to initialise a new process
\texttt{worker}, which is shown in listing \ref{list:yellow_toggler}.
...
Lines 36-39 in listing \ref{list:yellow_toggler} show how the port
\texttt{yellow\_print} is initialised and lines 43-47 show how messages are sent
to the \texttt{stdio\_sys} partition. For every message the yellow led is
toggled 10 times with 100 milliseconds inbetween (lines 49-52). In a complete
...
accessed through an APEX call, with added integration with the process scheduler.
...
This is done because the MPU is not currently being used to protect memory.

...
\begin{labeling}{HAL and CMSIS}
	\item [\textbf{Partitions}]
		...
		partitions are achieving this separated from one another.
	\item [\textbf{APEX}]
...
interface, the STLINK-V2 in-circuit debugger/programmer is used, through
the USB.
...
on a computer, one could use a USB-to-serial adapter. This would then
create a virtual COM (Communication port), that could be accessed using
a serial console such as PuTTY.
\\\\
The Mini-M4 board accommodates the 5 wires from the JTAG,
the 2 wires from the UART, and receives power through the built-in
mini-USB port.
...
perform atomical operations on bits.
...
The figure shows the intended schema for a fully compliant ARINC 653 implementation as found in the ARINC standard.  The greyed out areas, are the aspects of the open653 schema not yet implemented. 

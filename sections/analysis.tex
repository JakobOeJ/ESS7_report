...
This chapter includes information about the standard that is going to be
implemented and the different pieces of hardware suitable for the purpose.
After this, the arguments for choosing one platform are presented, as well
as a quick review over the other existing implementations.
...
This means that the applications are scheduled in a manner that would allow them 
to respond quickly and predictable.
...
\subsection{Overview}
ARINC 653 is a software specification that can run multiple applications on the same hardware, while providing resources in the context of an IMA system.
...
\\\\
Focussing on core OS features, only a few basic peripherals
...
That, and the availability of sample code implementations and a HAL library,
means that the MINI-M4 for STM32 is the hardware platform being used and referred to
in the duration of this project.
\todo{leads to the choice...}
\todo{Why have a HAL, why is it good to have}
...
for some years at Aalborg University. 
\todo{reference}
The choice of hardware was
slightly influenced by this, as well as the tools and libraries used 
throughout the project. The big advantage of this, is having the 
experience and knowledge to give a quick start
...
\todo[inline, color=green]{Ada / c / C++}
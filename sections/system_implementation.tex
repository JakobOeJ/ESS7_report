...
\subsection{MPU}
As the regions are power of 2, as shown in the picture, the bigger is the partition the more likely it 
...
Let's take as an example 5 partitions with the sizes of 20, 65, 120, 80 and 40bits.\\
\includegraphics[width=15cm]{partitions/partitions1.png}
If we just putted them in the most suited region, we would have to reserve 480bits.\\
\includegraphics[width=15cm]{partitions/partitions3.png}
But if we first allocate the bigger partitions and try to fullfill the subreagions not occupied with the smaller ones we can reduce the memory needed, like shown in the figure, using only 384bits.\\
\includegraphics[width=15cm]{partitions/partitions4.png}
In case that a certain partition doesn't full fill the subregions we 
update the partition size with that free space, 
instead of being always empty. That can also help to avoid some errors, for example if the initial 
defined size wasn't enough for the different processes allocated by that partition.\\
...
using the window watchdog timer

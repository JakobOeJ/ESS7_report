...
The features ``Health Monitor'' and ``Intrapartition Communication'' has been
excluded from this project to contain to limit the scope to a manageable amount... FIX ME!
...
the ARM instruction set \cite{arm_architecture}. Additional information
...
A simplified design of queuing ports and sampling ports is designed for \OSname .
...
The necessary attributes for queuing ports specifically are the following:
\begin{itemize}
...
The necessary attributes for sampling ports specifically are the following:
\begin{itemize}
...
the most partitions possible in a limited region of memory and by expanding the partition size fulfilling 
...
specify the configuration of the partitions memory; 
the config includes the amount of memory it requires,
the type of data the stored, how the data is accessed 
...
more Window\_Schedules and four attributes which correlate 
...
specify the max time a partition can use in one cycle for 
its Window\_Schedules to complete. It is unused currently as 
there is not appropriate error handling to make use of it and 
Window\_Schedules attributes are sufficient for scheduling.
\\
...
of the channel. An attribute is missing from the original 
ARINC 653 schema which is Procedure. Procedure is meant to 
be used by ports to interact with drivers.
...
contains one or more processes which are initialised during run-time.
...
groups of information about every partition, which makes sense to separate for
different parts of the system (eg. only the scheduler needs to know about a
partitions execution window information).\\

While information about the overall partition is defined statically, its
processes are defined and created at runtime and as such every partition needs
some memory allocated to store this information. A starting process is created
by default using the partitions entry-point(ref here?), which is then allowed to
spawn any additional processes.\\

Processes just like partitions are collections of information. Processes holds
information about its own state and how they are to be scheduled.

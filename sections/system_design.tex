...
>>>>>>> 0a234b3176a492d6162286dda28287cb71baa917
	\item \textbf{Drivers}
...
Each region is splited in 8 subregions, in which, some of those subregions are given to a specific partition. In order to optimize the memory used we can insert different partitions in the same region. When we define the memory for 1 partition we can try to fit more smaller partitions if there are some free sub regions so we have less non used memory.

...
For the choice of the language used to program the OS environment it was important to take in concern some 
especifications, such as, being a low level language, in order to have a better control of the processes 
and memory, being imperative, in order to follow the instructions exactly how they are suppose and the
previous experience of the group members working with that language.\\
The three languages taken in consideration were Ada, C and C++. \\
Ada was created during the 70's by the 
Defense Department of the USA, in order to avoid having many different languages used in the different 
departments. It is a high level language, and with high crytical control, making it safe to be used in 
airplanes software. \\
C language is a low level language, also created during the 70's, being one of the most popular languages 
in the world nowadays. Is a low level language, allowing a good memory and instructions control avoiding
some errors or bugs created by laguage libraries. It is also the most common used by the group members.\\
C++ is a multi paradigm language, mixing C with Java, created in the 90's. It is more an object oriented
language, allowing the creation of classes and heritage between classes.  
For the XML it can be used an higer level language, since only the output matters, not being really 
important how the compiler creates the files. We could use Python as an example, since is a laguage easy 
to work with.\\
\todo[inline, color=green]{Ada / c / C++ - Is this placed right?}
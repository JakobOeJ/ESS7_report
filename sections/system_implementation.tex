...
\captionof{figure}{Partition memory distribuition in different regions}
\label{fig:simple_system}
\end{figure}
...
\captionof{figure}{Partition memory distribuition in the same regions}
\label{fig:simple_system}
\end{figure}
...
\begin{algorithm}[H]
\tcc{excluding implicit idle peroids}
...
\While{windows in schedule < $totalWindows$}{
	...
	\If{matching window is found}{
		...
		insert window for $partiton$ into schedule\;
...
Listing \ref{list:port_struct} shows how every port has the four following common variables:
...
hence the use of an object wit the type \texttt{circBuf\_t} to hold additional data,
...
and wont be covered in this report.
\\\\
...
meaning that one can access its members directly instead of as a substruct.
While this is not strictly a native C feature it is enabled in GCC with the \texttt{-fms-extensions} flag
and is only used to make easier to read and edit.
\\\\
While a the ports.h file is used to declare all information about the ports,
The functions to work on sampling ports and queuing ports are located in src/kernel/sampling\_port.c and src/kernel/queuing\_ports.c respectively.

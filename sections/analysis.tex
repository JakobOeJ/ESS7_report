...
the standard, the hardware that is going to be used as well as the programming
language.
...
be changed later on.
\\\\
\textbf{Interpatition communication}
...
\section{Other existing standards}
While \arinc{} defines the architecture of a system for safety-critical
avionics applications, there are other ways to categorize how such a system
...
software assurance in safety-critical software in avionics systems.
Use of the standard can determine the reliability of the software in an airborne environment.
There is a \textit{safety assessment process} and \textit{hazard analysis} to determine the result of failure conditions on the system.
The final result of the standard applied to the software system will give the system a \textit{software level} of A to E compliance,
...
\subsection{Certification of \arinc{}}
One might wonder what \arinc{} has to do with DO-178B. 
...
These are divided into and open source OSs and COTS\footnote{Commercial of the Shelf} OSs.
An existing platform could be used as a starting point for 
the implementation of \OSname{}.

\textbf{ARLX}\cite{arlx}
is the only open-source platform available. It is a prototype hypervisor
for ARINC 653 that uses Linux to partition the system.
...
is marketed as \textit{The Industry's Leading Operating System for Embedded devices}.
...
least four results. The commercial OSs are excluded from the start since
...
different options for hardware have been investigated in this section.
...
\textbf{Raspberry Pi 3,} probably the most well known platform,
